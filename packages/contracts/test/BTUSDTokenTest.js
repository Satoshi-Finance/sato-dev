...
contract('BTUSDToken', async accounts => {
  ...
  const testCorpus = ({ withProxy = false }) => {
    ...
    it("name(): returns the token's name", async () => {
      ...
      assert.equal(name, "btUSD the Stable Coin")
    })

    it("symbol(): returns the token's symbol", async () => {
      const symbol = await debtTokenTester.symbol()
      assert.equal(symbol, "btUSD")
    })
    ...
    if (!withProxy) {
      ...
      it('permits and emits an Approval event (replay protected)', async () => {
        ...
        await assertRevert(debtTokenTester.permit(
          approve.owner, approve.spender, approve.value,
          deadline, v, r, s), 'BTUSD: invalid signature')

        // Check that the zero address fails
        await assertRevert(debtTokenTester.permit('0x0000000000000000000000000000000000000000',
                                                  approve.spender, approve.value, deadline, '0x99', r, s), 'BTUSD: invalid signature')
      })
      ...
      it('permits(): fails with expired deadline', async () => {
        ...
        const { v, r, s, tx } = await buildPermitTx(deadline)
        await assertRevert(tx, 'BTUSD: expired deadline')
      })
      ...
      it('permits(): fails with the wrong signature', async () => {
        ...
        await assertRevert(tx, 'BTUSD: invalid signature')
      })
    }
  }
