...
* Within the first year from deployment, the deployer of the SATOToken may transfer SATO only to valid 
* LockupContracts, and no other addresses (this is enforced in SATOToken.sol's transfer() function).
* 
* The above two restrictions ensure that until one year after system deployment, SATO tokens originating from Liquity AG cannot 
...
contract LockupContract {
    ...
    event LockupContractEmptied(uint _withdrawal);
    ...
    function withdrawSATO() external {
        _requireCallerIsBeneficiary();
        _requireLockupDurationHasPassed();

        ISATOToken satoTokenCached = satoToken;
        uint SATOBalance = satoTokenCached.balanceOf(address(this));
        satoTokenCached.transfer(beneficiary, SATOBalance);
        emit LockupContractEmptied(SATOBalance);
    }
