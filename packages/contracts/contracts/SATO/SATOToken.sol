...
* 
*
*  --- Functionality added specific to the SATOToken ---
* 
* 1) Transfer protection: blacklist of addresses that are invalid recipients (i.e. core Liquity contracts) in external 
* transfer() and transferFrom() calls. The purpose is to protect users from losing tokens by mistakenly sending SATO directly to a Liquity
...
contract SATOToken is CheckContract, ISATOToken {
    ...
    string constant internal _NAME = "SATO";
    string constant internal _SYMBOL = "SATO";
    ...
    function sendToStaking(address _sender, uint256 _amount) external override {
        _requireCallerIsSATOStaking();
        if (_isFirstYear()) { _requireSenderIsNotMultisig(_sender); }  // Prevent the multisig from stakings
        _transfer(_sender, stakingAddress, _amount);
    }
    ...
    (
        ...
        override 
    {            
        require(deadline >= now, 'SATO: expired deadline');
        bytes32 digest = keccak256(abi.encodePacked('\x19\x01', 
                         domainSeparator(), keccak256(abi.encode(
                         _PERMIT_TYPEHASH, owner, spender, amount, 
                         _nonces[owner]++, deadline))));
        address recoveredAddress = ecrecover(digest, v, r, s);
        require(recoveredAddress == owner, 'SATO: invalid signature');
    ...
    function _requireValidRecipient(address _recipient) internal view {
        require(
            ...
            "SATO: Cannot transfer tokens directly to the token contract or the zero address"
        );
        require(
            _recipient != communityIssuanceAddress &&
            _recipient != stakingAddress,
            "SATO: Cannot transfer tokens directly to the community issuance or staking contract"
        );
    }
    ...
    function _requireRecipientIsRegisteredLC(address _recipient) internal view {
        ...
        "SATOToken: recipient must be a LockupContract registered in the Factory");
    }

    function _requireSenderIsNotMultisig(address _sender) internal view {
        require(_sender != multisigAddress, "SATOToken: sender must not be the multisig");
    }

    function _requireCallerIsNotMultisig() internal view {
        require(!_callerIsMultisig(), "SATOToken: caller must not be the multisig");
    }

    function _requireCallerIsSATOStaking() internal view {
         require(msg.sender == stakingAddress, "SATOToken: caller must be the SATOStaking contract");
    }
