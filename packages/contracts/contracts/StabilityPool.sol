 ...
 *
 * When a trove is liquidated, then depending on system conditions, some of its debt gets offset within the Stability Pool:  
 * that is, the offset debt evaporates, and an equal amount of debt tokens in the Stability Pool is burned.
 *
 * Thus, a liquidation causes each depositor to receive a debt deposit reduction, in proportion to their deposit as a share of total deposits.
 ...
 * as 0, since it is now less than 1e-9'th of its initial value (e.g. a deposit of 1 billion debt token has depleted to < 1 debt token).
 *
 *
 ...
 *
 *
 * --- SATO ISSUANCE TO STABILITY POOL DEPOSITORS ---
 *
 * An SATO issuance event occurs at every deposit operation, and every liquidation.
 *
 * Each deposit is tagged with the address of the front end through which it was made.
 *
 * All deposits earn a share of the issued SATO in proportion to the deposit as a share of total deposits. The SATO earned
 * by a given deposit, is split between the depositor and the front end through which the deposit was made, based on the front end's kickbackRate.
 *
 * We use the same mathematical product-sum approach to track SATO gains for depositors, where 'G' is the sum corresponding to SATO gains.
...
contract StabilityPool is LiquityBase, Ownable, CheckContract, IStabilityPool {
    ...
    */
    ...
    /*
    * Similarly, the sum 'G' is used to calculate SATO gains. During it's lifetime, each deposit d_t earns a SATO gain of
    *  ( d_t * [G - G_t] )/P_t, where G_t is the depositor's snapshot of G taken at time t when  the deposit was made.
    *
    *  SATO reward events occur are triggered by depositor operations (new deposit, topup, withdrawal), and liquidations.
    *  In each case, the SATO reward is issued (i.e. G is updated), before other state changes are made.
    */
    mapping (uint128 => mapping(uint128 => uint)) public epochToScaleToG;

    // Error tracker for the error correction in the SATO issuance calculation
    uint public lastSATOError;
    ...
    event StabilityPoolBTUSDBalanceUpdated(uint _newBalance);
    ...
    event ETHGainWithdrawn(address indexed _depositor, uint _ETH, uint _debtLoss);
    event SATOPaidToDepositor(address indexed _depositor, uint _amt);
    event SATOPaidToFrontEnd(address indexed _frontEnd, uint _amt);
    event EtherSent(address _to, uint _amount);
    event WithdrawRequest(address indexed _depositor, uint _time, uint _amount);
    ...
    /*  provideToSP():
    *
    * - Triggers a SATO issuance, based on time passed since the last issuance. The SATO issuance is shared between *all* depositors and front ends
    * - Tags the deposit with the provided front end tag param, if it's a new deposit
    * - Sends depositor's accumulated gains (SATO, ETH) to depositor
    * - Sends the tagged front end's accumulated SATO gains to the tagged front end
    ...
    */
    function provideToSP(uint _amount, address _frontEndTag) external override {
        ...
        _triggerSATOIssuance(communityIssuanceCached);
        ...
        // First pay out any SATO gains
        address frontEnd = deposits[msg.sender].frontEndTag;
        _payOutSATOGains(communityIssuanceCached, msg.sender, frontEnd);
        ...
        emit ETHGainWithdrawn(msg.sender, depositorETHGain, debtLoss); // debt Loss required for event log

        _sendETHGainToDepositor(depositorETHGain);
    }
	
    /// @dev request a withdrawal from Stability Pool
    /// @dev ensure deposit is not zero to withdraw by first querying getCompoundedDebtDeposit()
    function requestWithdrawFromSP(uint _amount) external override {
        _requireSystemTCR();
        ...
        uint _ts = block.timestamp;
        withdrawReqAmount[msg.sender] = withdrawAmt;
        withdrawReqTime[msg.sender] = _ts;
        emit WithdrawRequest(msg.sender, _ts, withdrawAmt);
    }
    ...
    * - Sends all depositor's accumulated gains (SATO, ETH) to depositor
    * - Sends the tagged front end's accumulated SATO gains to the tagged front end
    ...
    */
    function withdrawFromSP(uint _amount) external override {
        ...
        (uint compoundedDebtDeposit, uint debtToWithdraw) = _capWithdrawAmount(msg.sender, _amount);
        uint debtLoss = initialDeposit.sub(compoundedDebtDeposit); // Needed only for event log
    ...
    * - Triggers a SATO issuance, based on time passed since the last issuance. The SATO issuance is shared between *all* depositors and front ends
    * - Sends all depositor's SATO gain to  depositor
    * - Sends all tagged front end's SATO gain to the tagged front end
    ...
    * - Updates snapshots for deposit and tagged front end stake */
    function withdrawETHGainToTrove() external override {
        ...
        emit ETHGainWithdrawn(msg.sender, depositorETHGain, debtLoss);
        ...
        collateral.approve(address(borrowerOperations), depositorETHGain);
        borrowerOperations.moveCollGainToTrove(msg.sender, depositorETHGain);
    }
	
    function _capWithdrawAmount(address _depositor, uint _amount) internal view returns(uint, uint){	
    ...
    */
    function offset(uint _debtToOffset, uint _collToAdd) external override {
        ...
        _triggerSATOIssuance(communityIssuance);

        (uint ETHGainPerUnitStaked,
            uint debtLossPerUnitStaked) = _computeRewardsPerUnitStaked(_collToAdd, _debtToOffset, totalDebt);

        _updateRewardSumAndProduct(ETHGainPerUnitStaked, debtLossPerUnitStaked);  // updates S and P
    ...
    function _computeRewardsPerUnitStaked(
        ...
        internal
        returns (uint ETHGainPerUnitStaked, uint debtLossPerUnitStaked)
    {
        /*
        ...
        */
        ...
        if (_debtToOffset == _totalDebtDeposits) {
            ...
            uint debtLossNumerator = _debtToOffset.mul(DECIMAL_PRECISION).sub(lastLUSDLossError_Offset);
            /*
            ...
            */
            ...
            lastLUSDLossError_Offset = (debtLossPerUnitStaked.mul(_totalDebtDeposits)).sub(debtLossNumerator);
        }

        ETHGainPerUnitStaked = ETHNumerator.div(_totalDebtDeposits);
        lastETHError_Offset = ETHNumerator.sub(ETHGainPerUnitStaked.mul(_totalDebtDeposits));

        return (ETHGainPerUnitStaked, debtLossPerUnitStaked);
    }

    // Update the Stability Pool reward sum S and product P
    function _updateRewardSumAndProduct(uint _ETHGainPerUnitStaked, uint _debtLossPerUnitStaked) internal {
        uint currentP = P;
        uint newP;
        ...
        */
        ...
        // If multiplying P by a non-zero product factor would reduce P below the scale boundary, increment the scale
            ...
            newP = newP > 0? newP : 1;
            uint128 _newScale = currentScaleCached.add(1);
    ...
    function _moveOffsetCollAndDebt(uint _collToAdd, uint _debtToOffset) internal {
        ...
        // Cancel the liquidated debt within the stability pool
    ...
    function _decreaseDebt(uint _amount) internal {
        ...
        emit StabilityPoolBTUSDBalanceUpdated(newTotalDebtDeposits);
    }
    ...
    */
    ...
    /*
    * Calculate the SATO gain earned by a deposit since its last snapshots were taken.
    * Given by the formula:  SATO = d0 * (G - G(0))/P(0)
    * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
    * d0 is the last recorded deposit value.
    */
    function getDepositorSATOGain(address _depositor) public view override returns (uint) {
        uint initialDeposit = deposits[_depositor].initialValue;
        if (initialDeposit == 0) {return 0;}
    ...
    /*
    * Return the SATO gain earned by the front end. Given by the formula:  E = D0 * (G - G(0))/P(0)
    * where G(0) and P(0) are the depositor's snapshots of the sum G and product P, respectively.
    *
    * D0 is the last recorded value of the front end's total tagged deposits.
    */
    function getFrontEndSATOGain(address _frontEnd) public view override returns (uint) {
        ...
        uint SATOGain = frontEndShare.mul(_getSATOGainFromSnapshots(frontEndStake, snapshots)).div(DECIMAL_PRECISION);
        return SATOGain;
    }
    ...
    */
    ...
    // --- Sender functions for debt deposit, collateral gains and SATO gains ---
    ...
    function _sendETHGainToDepositor(uint _amount) internal {
        ...
        require(collateral.transfer(msg.sender, _amount), "StabilityPool: sending ETH failed");
    }
    ...
    function _payOutSATOGains(ICommunityIssuance _communityIssuance, address _depositor, address _frontEnd) internal {
        // Pay out front end's SATO gain
        if (_frontEnd != address(0)) {
            uint frontEndSATOGain = getFrontEndSATOGain(_frontEnd);
            _communityIssuance.sendSATO(_frontEnd, frontEndSATOGain);
            emit SATOPaidToFrontEnd(_frontEnd, frontEndSATOGain);
        }

        // Pay out depositor's SATO gain
        uint depositorSATOGain = getDepositorSATOGain(_depositor);
        _communityIssuance.sendSATO(_depositor, depositorSATOGain);
        emit SATOPaidToDepositor(_depositor, depositorSATOGain);
    }
    ...
    /// @dev called in _moveOffsetCollAndDebt()
